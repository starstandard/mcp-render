...
function jsonToolResult(data: Record<string, unknown>, label?: string) {
  return {
...
function registerTools(target: McpServer) {
  target.tool(
    'getApiOverview',
  ...
  target.tool(
    ...
    {},
    async () => {
      const data = { domains: loadedApis.map((api) => api.domain_name) };
      return jsonToolResult(data, 'Available domains');
    }
  );
...
const claudeServer = new McpServer({ name: SERVER_NAME, version: '2.0.0' });
const openAiServer = new McpServer({ name: SERVER_NAME, version: '2.0.0' });

registerTools(claudeServer);
registerTools(openAiServer);
...
async function handleClaudeMcpRequest(req: Request, res: Response) {
  ...
  try {
    await claudeServer.connect(transport);
...
async function handleOpenAiMcpRequest(req: Request, res: Response) {
  if (!openAiTransport) {
    ...
    await openAiServer.connect(openAiTransport);
  }
